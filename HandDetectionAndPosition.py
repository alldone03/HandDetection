...
# pixelx = 1280
# pixely = 720
pixelx = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
pixely = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
print(pixelx, pixely)

dataBox = [[[0, 0], [int(pixelx / 3), int(pixely / 3)]],
           [[int(pixelx / 3), 0], [int((pixelx / 3) * 2), int(pixely / 3)]],
           [[int((pixelx / 3) * 2), 0], [int((pixelx / 3) * 3), int(pixely / 3)]],

           [[0, int(pixely / 3)], [int(pixelx / 3), int((pixely / 3) * 2)]],
           [[int(pixelx / 3), int(pixely / 3)],
            [int((pixelx / 3) * 2), int((pixely / 3) * 2)]],
           [[int((pixelx / 3) * 2), int(pixely / 3)],
            [int((pixelx / 3) * 3), int(pixely / 3) * 2]],

           [[0, int((pixely / 3) * 2)],
            [int(pixelx / 3), int((pixely / 3) * 3)]],
           [[int((pixelx / 3)), int((pixely / 3) * 2)],
            [int((pixelx / 3) * 2), int((pixely / 3) * 3)]],
           [[int((pixelx / 3) * 2), int(pixely / 3) * 2], [int((pixelx / 3) * 3), int((pixely / 3) * 3)]]]
with pyvirtualcam.Camera(width=int(pixelx), height=int(pixely), fps=30) as cam:
    with mp_hands.Hands(
            model_complexity=0,
        ...
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    cv2.circle(
                        image, dataPointlandmark[8], 2, (28, 215, 249), 2)
                dataBoxOn = []
